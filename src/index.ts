...
console.log(
    ...
    )}  ${chalk.bgCyanBright.black("--> By Jayateerth Dambal")}`
);
program
    .name('string-calculator')
    .description(
        chalk.greenBright('A CLI tool for testing the String Calculator built with TDD')
    )
    .version('1.0.0');
...
program
    ...
    .action((input: string) => {
        try {
            ...
            console.log(
                `${chalk.bold.green('Result:')} ${chalk.bgBlueBright.black(
                    ` ${result} `
                )}\n`
            );
        } catch (error) {
            console.error(
                `\n${chalk.redBright('Error:')} ${(error as Error).message}\n`
            );
        }
    });
...
program
    ...
    .action(() => {
        ...
        console.log(
            `${chalk.cyan('Benchmarking')} ${chalk.yellow(iterations)} iterations on a large input...\n`
        );
        ...
        console.log(
            `${chalk.green('Average Execution Time:')} ${chalk.yellow(
                `${avgTime.toFixed(2)}ms`
            )}`
        );
        console.log(
            `${chalk.green('Total Time for')} ${chalk.yellow(
                iterations
            )} ${chalk.green('iterations:')} ${chalk.yellow(
    ...
    .action(() => {
        ...
        testCases.forEach(({ input, expected, description }, index) => {
            try {
                ...
                console.log(
                    `${chalk.bold.green(`Test ${index + 1}:`)} ${chalk.cyan(description)}`
                );
                console.log(
                    `${chalk.bold.yellow('Input:')} ${chalk.white(input)}`
                );
                console.log(
                    `${chalk.bold.yellow('Expected:')} ${chalk.white(expected)}`
                );
                console.log(
                    `${chalk.bold.yellow('Result:')} ${success
                ...
                console.log(
                    `${chalk.bold.yellow('Error:')} ${success
